...
from logic.bomb_baby import answer


class TestBombBaby(unittest.TestCase):
    """Challenge 3
    Bomb, Baby!
    ===========
    ...
    device. Too few, and the device might survive. Too many, and you might overload the mass capacitors and
    create a singularity at the heart of the space station - not good!

    And finally, you were only able to smuggle one of each type of bomb - one Mach, one Facula - aboard the ship
    when you arrived, so that's all you have to start with. (Thus it may be impossible to deploy the bombs to
    ...
    def test_three(self):
        response = answer("2", "4")
        self.assertEqual(response, "impossible")
