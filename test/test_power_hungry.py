...
from logic.power_hungry import answer
...
class TestPowerHungry(unittest.TestCase):
    ...
    wreaks havoc on the solar panels. You and your team of henchmen has been assigned to repair the solar panels, but
    you can't take them all down at once without shutting down the space station (and all those pesky life support
    systems!).
    ...
    maximum output of each array actually is. Write a function answer(xs) that takes a list of integers representing
    the power output levels of each panel in an array, and returns the maximum product of some non-empty subset of those
    numbers. So for example, if an array contained panels with power output levels of [2, -3, 1, 0, -5], then the
    maximum product would be found by taking the subset: xs[0] = 2, xs[1] = -3, xs[4] = -5, giving the product
    ...
    def test_example_1(self):
        response = answer([2, 0, 2, 2, 0])
        self.assertEqual(response, "8")

    @print_time
    def test_example_2(self):
        response = answer([-2, -3, 4, -5])
        self.assertEqual(response, "60")
